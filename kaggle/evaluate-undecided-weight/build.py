"""Build the CUDA evaluator for the uncertainty-weighted-loss pilot
(docs/15-claude-score-improvement-strategy.md, experiment 2). Inference
only - no retraining, no submission.

Scores every checkpoint the two-fold pilot produced (train-uw-control,
train-uw-025, train-uw-000; folds 0,1 only; best + snapshot epochs 16/20/24
each) on CUDA, so the three UNDECIDED_WEIGHT settings are compared under the
same backend rather than each leg's own TPU-recorded score - experiment 1
already established those two backends can disagree.

Usage: python kaggle/evaluate-undecided-weight/build.py OUTPUT_DIRECTORY
...
''' + source[end:]
    ...
    ast.parse(source)
    metadata = {
        "id": "sibusisokhumalo11/knee-evaluate-undecided-weight",
        "title": "knee-evaluate-undecided-weight",
        "code_file": "script.py",
        "language": "python",
        "kernel_type": "script",
        "is_private": True,
        ...
        "dataset_sources": [
            ...
            "sibusisokhumalo11/knee-slot-uw-pilot-v1",
        ],
    ...
    print(f"Built uncertainty-weight evaluator -> {output}")
