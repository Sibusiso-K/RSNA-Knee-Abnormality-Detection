"""CUDA evaluator for docs/15 step 4: translation test-time augmentation on
the deployed small+base 24-epoch checkpoints (the ones behind 0.891).
All five folds, each scored only by its own held-out checkpoints. Inference
only - no training, no submission.

Usage: python kaggle/evaluate-tta/build.py OUTPUT_DIRECTORY
...
''' + source[end:]
    ...
    ast.parse(source)
    metadata = {
        "id": "sibusisokhumalo11/knee-evaluate-tta",
        "title": "knee-evaluate-tta",
        "code_file": "script.py",
        "language": "python",
        "kernel_type": "script",
        "is_private": True,
    ...
    print(f"Built TTA evaluator -> {output}")
