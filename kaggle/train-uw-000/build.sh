#!/usr/bin/env bash
# Experiment 2 (docs/15), candidate leg: UNDECIDED_WEIGHT=0.0 (full mask),
# folds 0,1 only. Identical to train-uw-control except this one line - see
# that config for the full rationale.
#
# 0.0 fully masks the undecided cells out of the loss. docs/15 warns this
# is the riskier of the two candidates: missingness in these reports is not
# random, so masking can teach the model to ignore an entire mode of the
# data, not just reduce noise. Measure it, do not assume it is safe because
# 0.25 is.
set -euo pipefail
sed -e 's|os.environ.get("HEAD", "slot")|os.environ.get("HEAD", "xattn")|' \
    ...
    -e 's|os.environ.get("UNDECIDED_WEIGHT", "1.0")|os.environ.get("UNDECIDED_WEIGHT", "0.0")|' \
...
grep -q 'os.environ.get("UNDECIDED_WEIGHT", "0.0")'  "$2/script.py" || { echo "UNDECIDED_WEIGHT patch missed" >&2; exit 1; }
