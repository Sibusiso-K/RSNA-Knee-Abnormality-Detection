"""CUDA evaluator for docs/20 job 1: do existing epoch-24/epoch-20 snapshot
checkpoints improve on the 0.893 TTA anchor, with no retraining?

All five folds, each study scored only by its held-out checkpoints, under
the exact production five-view translation TTA (src.model.tta.tta_probs).
Inference only - no training, no submission.

Usage: python kaggle/evaluate-snap-tta/build.py OUTPUT_DIRECTORY
...
''' + source[end:]
    ...
    ast.parse(source)
    metadata = {
        "id": "sibusisokhumalo11/knee-evaluate-snap-tta",
        "title": "knee-evaluate-snap-tta",
        "code_file": "script.py",
        "language": "python",
        "kernel_type": "script",
        "is_private": True,
        ...
        "dataset_sources": [
            ...
            "sibusisokhumalo11/knee-slot-6slice-24ep-v1",
    ...
    print(f"Built snap-TTA evaluator -> {output}")
