...
N_FOLDS = 5
# NOT str(IMG): src.data.slots.IMG is 336, and defaulting to it would
# silently downscale the 448 cache back to 336 and void the experiment.
TRAIN_SIZE = int(os.environ.get("SIZE", "448"))
...
# The whole point of this run is the pixel scale, so a mis-attached cache must
# stop the run rather than quietly produce a number that looks comparable to
# the 336 px baseline and is not.
if cache.shape[-1] != TRAIN_SIZE:
    raise SystemExit(
        f"cache is {cache.shape[-1]} px but TRAIN_SIZE is {TRAIN_SIZE}. "
        f"Attach knee-cache-448x6-s0/s1, or set SIZE to match."
    )
log(f"pixel scale: {130.0 / cache.shape[-1]:.4f} mm/px "
    f"(baseline 336 px = {130.0 / 336:.4f} mm/px)")
