...
_slots.N_GROUP = 2                     # GROUP(3) x N_GROUP(2) = 6 slices
N_SLICE = _slots.GROUP * _slots.N_GROUP
_cache.N_SLICE = N_SLICE
assert _cache.N_SLICE == N_SLICE == 6, "N_SLICE override failed"

# --- 448 px override -----------------------------------------------------
# Patch the module that USES IMG (src.data.cache imports it into its own
# namespace at import time) as well as this script's copy. 448 = 14 x 32, so it
# stays patch-14 clean for DINOv2.
IMG = 448
_cache.IMG = IMG
_slots.IMG = IMG
