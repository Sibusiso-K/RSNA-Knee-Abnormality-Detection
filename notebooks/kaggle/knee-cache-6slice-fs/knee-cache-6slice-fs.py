...
# --- fat-suppression from the CSV, not from _FATSAT_RX --------------------
# knee-fatsat-scan measured this. The column named Fluid_Sensitive actually
# encodes FAT SUPPRESSION: it agrees with the header-derived fat-sat value on
# 97.31% of series, versus 80.19% read as fluid-sensitivity. (Fluid_Sensitive
# and Fat_Suppression are identical in all 24,371 rows, so they carry one bit;
# that is which bit.)
#
# _FATSAT_RX misses 592 series the CSV gets right, from three causes: the
# underscore bug (fs, stir, tirm cannot match between
# underscores), missing vocabulary ("we" = water excitation, Philips
# SMART FAT), and series whose only evidence is ScanningSequence == IR.
# Effect: 255 studies (5.79%) change slot assignment, concentrated on
# AX_FLUID_FS (90.38% -> 94.26% filled), a prior slot for PF OA.
#
# Patched at probe_study, not classify_weighting: the flag is per SERIES and
# classify_weighting only receives text. Wrapping the original rather than
# reimplementing it keeps this in step with src/data/cache.py - a forked copy
# is how train and inference drift apart.
FATSAT_OF = dict(zip(series_meta["SeriesInstanceUID"],
                     series_meta["Fat_Suppression"]))
_orig_probe_study = _cache.probe_study
FATSAT_OVERRIDES = {"changed": 0, "seen": 0}


def _probe_study_csv_fatsat(series_dir, study_uid, plane_of):
    infos, files_of, fingerprint = _orig_probe_study(series_dir, study_uid, plane_of)
    for info in infos:
        flag = FATSAT_OF.get(info.uid)
        # An absent flag keeps the header verdict: a missing value is not
        # evidence of "not fat-suppressed".
        if flag is None or pd.isna(flag):
            continue
        FATSAT_OVERRIDES["seen"] += 1
        want = bool(flag == 1)
        if info.fatsat != want:
            FATSAT_OVERRIDES["changed"] += 1
        info.fatsat = want
    return infos, files_of, fingerprint


_cache.probe_study = _probe_study_csv_fatsat
print("[cfg] fat-suppression sourced from CSV Fat_Suppression, not _FATSAT_RX",
      flush=True)
